“”“
在LSTM1的基础上，真正实现了LSTM中同一个batch的并行训练，
训练速度与源码直接调用nn LSTM速度相当，训练效果也相当 甚至略好
”“”
import os
import math
import torch
...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        self.W_xf = nn.Linear(in_features=emb_size,out_features=n_hidden,bias=False)
        self.b_xf = nn.Parameter(torch.ones([n_hidden]))
        self.W_hf = nn.Linear(in_features=n_hidden,out_features=n_hidden,bias=False)
        self.b_hf = nn.Parameter(torch.ones([n_hidden]))
        self.ft = nn.Sigmoid()

        self.W_xi = nn.Linear(in_features=emb_size, out_features=n_hidden, bias=False)
        self.b_xi = nn.Parameter(torch.ones([n_hidden]))
        self.W_hi = nn.Linear(in_features=n_hidden, out_features=n_hidden, bias=False)
        self.b_hi = nn.Parameter(torch.ones([n_hidden]))
        self.it = nn.Sigmoid()

        self.W_xo = nn.Linear(in_features=emb_size, out_features=n_hidden, bias=False)
        self.b_xo = nn.Parameter(torch.ones([n_hidden]))
        self.W_ho = nn.Linear(in_features=n_hidden, out_features=n_hidden, bias=False)
        self.b_ho = nn.Parameter(torch.ones([n_hidden]))
        self.ot = nn.Sigmoid()

        self.W_xg = nn.Linear(in_features=emb_size, out_features=n_hidden, bias=False)
        self.b_xg = nn.Parameter(torch.ones([n_hidden]))
    ...
    def forward(self, X):
        ...
        hidden_state = torch.zeros(batch_size,n_hidden)
        cell_state = torch.zeros(batch_size,n_hidden)
        for x in X:
            # print("x.shape",x.shape)
            # print("hidden.shape",hidden_state.shape)
            i = self.it(self.W_xi(x) + self.b_xi + self.W_hi(hidden_state) + self.b_hi)
            f = self.ft(self.W_xf(x) + self.b_xf + self.W_hf(hidden_state) + self.b_hf)
            o = self.ot(self.W_xo(x) + self.b_xo + self.W_ho(hidden_state) + self.b_ho)
            g = self.gt(self.W_xg(x) + self.b_xg + self.W_hg(hidden_state) + self.b_hg)
            cell_state = f * cell_state + i * g
            hidden_state = o * self.ht(cell_state)

        model = self.W(hidden_state) + self.b  # model : [batch_size, n_class]
        return model
