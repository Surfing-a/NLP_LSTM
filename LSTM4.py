"""
在这个文件中主要目的是将forward函数中的训练改为并行进行
先后尝试了 线程 进程池 父子进程
最后因为多进程全局变量的共享和关闭tensor自动求梯度导致无法正确进行反向传播而暂停
"""
...
import threading
from multiprocessing import Pool,Queue,Value,Array,Process
...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        self.W_f2 = nn.Linear(in_features=n_hidden+n_hidden, out_features=n_hidden, bias=False)
        ...
        self.W_i2 = nn.Linear(in_features=n_hidden+n_hidden, out_features=n_hidden, bias=False)
        ...
        self.W_o2 = nn.Linear(in_features=n_hidden+n_hidden, out_features=n_hidden, bias=False)
        ...
        self.W_g2 = nn.Linear(in_features=n_hidden+n_hidden, out_features=n_hidden, bias=False)
    ...
    def first_layer(self, q, x):
        # global h_t1, c_t1,share_lock_h1
        h_t1 = q.get(True)
        c_t1 = q.get(True)
        print(h_t1)
    ...
    def second_layer(self, q, h_t1):
        # global h_t2, c_t2,share_lock_h2
        h_t2 = q.get(True)
        c_t2 = q.get(True)
        i_2 = self.it2(self.W_i2(torch.cat((h_t2, h_t1), 1)) + self.b_i2)
        f_2 = self.ft2(self.W_f2(torch.cat((h_t2, h_t1), 1)) + self.b_f2)
        o_2 = self.ot2(self.W_o2(torch.cat((h_t2, h_t1), 1)) + self.b_o2)
        g_2 = self.gt2(self.W_g2(torch.cat((h_t2, h_t1), 1)) + self.b_g2)
    ...
    def forward(self, X):
        ...
        for x in X:
            ...
            if index  == 0:
                ...
                q1.put(h_t1)
                q1.put(c_t1)
                process1 = Process(target=self.first_layer,args=(q1,x))
                process1.start()
                q2.put(h_t2)
                q2.put(c_t2)
                process2 = Process(target=self.second_layer,args=(q2,h_t1))
                process2.start()
                # with Pool(2) as p:
                #     q1.put(h_t1)
                #     q1.put(c_t1)
                #     p.map(self.first_layer,[q1,x])
                #     q2.put(h_t2)
                #     q2.put(c_t2)
                #     p.map(self.second_layer,[q2,h_t1])
            index += 1

        model = self.W(h_t2) + self.b  # model : [batch_size, n_class]
        return model
